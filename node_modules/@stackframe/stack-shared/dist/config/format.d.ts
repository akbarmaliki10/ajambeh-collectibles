...
type _NormalizesTo<N> = N extends object ? (Config & { [K in OptionalKeys<N>]?: _NormalizesTo<N[K]> | null } & { [K in RequiredKeys<N>]: undefined extends N[K] ? _NormalizesTo<N[K]> | null : _NormalizesTo<N[K]> } & { [K in `${string}.${string}`]: ConfigValue }) : N;
...
//#endregion
export { Config, ConfigValue, NormalizationError, NormalizedConfig, NormalizedConfigValue, NormalizesTo, _NormalizesTo, assertNormalized, assertValidConfig, getInvalidConfigReason, isNormalized, isValidConfig, normalize, override, removeKeysFromConfig };
