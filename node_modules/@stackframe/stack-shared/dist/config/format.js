Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });
const require_chunk = require('../chunk-BE-pF4vm.js');
let ___utils_errors_js = require("../utils/errors.js");
let ___utils_objects_js = require("../utils/objects.js");

//#region src/config/format.ts
/**
* Note that a config can both be valid and not normalizable.
*/
function isValidConfig(c) {
	return getInvalidConfigReason(c) === void 0;
}
function getInvalidConfigReason(c, options = {}) {
	const configName = options.configName ?? "config";
	if (c === null || typeof c !== "object") return `${configName} must be a non-null object`;
	for (const [key, value] of Object.entries(c)) {
		if (value === void 0) continue;
		if (typeof key !== "string") return `${configName} must have only string keys (found: ${typeof key})`;
		if (!key.match(/^[a-zA-Z0-9_:$][a-zA-Z_:$0-9\-]*(?:\.[a-zA-Z0-9_:$][a-zA-Z_:$0-9\-]*)*$/)) return `All keys of ${configName} must consist of only alphanumeric characters, dots, underscores, colons, dollar signs, or hyphens and start with a character other than a hyphen (found: ${key})`;
		const reason = getInvalidConfigValueReason(value, { valueName: `${configName}.${key}` });
		if (reason) return reason;
	}
}
function getInvalidConfigValueReason(value, options = {}) {
	const valueName = options.valueName ?? "value";
	switch (typeof value) {
		case "string":
		case "number":
		case "boolean": break;
		case "object":
			if (value === null) break;
			else if (Array.isArray(value)) for (const [index, v] of value.entries()) {
				const elementValueName = `${valueName}[${index}]`;
				if (v === null) return `${elementValueName} is null; tuple elements cannot be null`;
				const reason = getInvalidConfigValueReason(v, { valueName: elementValueName });
				if (reason) return reason;
			}
			else {
				const reason = getInvalidConfigReason(value, { configName: valueName });
				if (reason) return reason;
			}
			break;
		default: return `${valueName} has an invalid value type ${typeof value} (value: ${value})`;
	}
}
function assertValidConfig(c) {
	const reason = getInvalidConfigReason(c);
	if (reason) throw new ___utils_errors_js.StackAssertionError(`Invalid config: ${reason}`, { c });
}
function override(c1, ...configs) {
	if (configs.length === 0) return c1;
	if (configs.length > 1) return override(override(c1, configs[0]), ...configs.slice(1));
	const c2 = configs[0];
	assertValidConfig(c1);
	assertValidConfig(c2);
	let result = c1;
	for (const key of Object.keys((0, ___utils_objects_js.filterUndefined)(c2))) result = Object.fromEntries(Object.entries(result).filter(([k]) => k !== key && !k.startsWith(key + ".")));
	return {
		...result,
		...(0, ___utils_objects_js.filterUndefined)(c2)
	};
}
/**
* Removes keys from a config override, using the same nested key logic as the `override` function.
* Resetting key "a.b" also resets "a.b.c" (and any other descendants).
* Handles both flat dot-notation keys and nested object keys.
*/
function removeKeysFromConfig(config, keysToRemove) {
	if (keysToRemove.length === 0) return config;
	const result = {};
	for (const [key, value] of Object.entries(config)) {
		if (value === void 0) continue;
		if (keysToRemove.some((k) => key === k || key.startsWith(k + "."))) continue;
		const childKeysToRemove = keysToRemove.filter((k) => k.startsWith(key + ".")).map((k) => k.slice(key.length + 1));
		if (childKeysToRemove.length > 0 && typeof value === "object" && value !== null && !Array.isArray(value)) {
			const cleaned = removeKeysFromConfig(value, childKeysToRemove);
			if (Object.keys(cleaned).length > 0) result[key] = cleaned;
		} else result[key] = value;
	}
	return result;
}
(void 0)?.test("removeKeysFromConfig(...)", ({ expect }) => {
	expect(removeKeysFromConfig({
		a: 1,
		b: 2
	}, ["a"])).toEqual({ b: 2 });
	expect(removeKeysFromConfig({
		"a.b": 1,
		"a.c": 2,
		d: 3
	}, ["a.b"])).toEqual({
		"a.c": 2,
		d: 3
	});
	expect(removeKeysFromConfig({
		"a.b": 1,
		"a.b.c": 2,
		"a.d": 3
	}, ["a.b"])).toEqual({ "a.d": 3 });
	expect(removeKeysFromConfig({
		"a.b": 1,
		"a.c": 2,
		"a.d": 3
	}, ["a"])).toEqual({});
	expect(removeKeysFromConfig({
		"teams.allowClientTeamCreation": true,
		"teams.createPersonalTeamOnSignUp": true
	}, ["teams"])).toEqual({});
	expect(removeKeysFromConfig({ teams: {
		allowClientTeamCreation: true,
		createPersonalTeamOnSignUp: true
	} }, ["teams.allowClientTeamCreation"])).toEqual({ teams: { createPersonalTeamOnSignUp: true } });
	expect(removeKeysFromConfig({ teams: { allowClientTeamCreation: true } }, ["teams.allowClientTeamCreation"])).toEqual({});
	expect(removeKeysFromConfig({ teams: {
		allowClientTeamCreation: true,
		createPersonalTeamOnSignUp: true
	} }, ["teams"])).toEqual({});
	expect(removeKeysFromConfig({ a: { b: {
		c: 1,
		d: 2
	} } }, ["a.b.c"])).toEqual({ a: { b: { d: 2 } } });
	expect(removeKeysFromConfig({
		"teams.allowClientTeamCreation": true,
		teams: { createPersonalTeamOnSignUp: true }
	}, ["teams.allowClientTeamCreation"])).toEqual({ teams: { createPersonalTeamOnSignUp: true } });
	expect(removeKeysFromConfig({
		"teams.allowClientTeamCreation": true,
		teams: { createPersonalTeamOnSignUp: true }
	}, ["teams"])).toEqual({});
	expect(removeKeysFromConfig({ teams: { "a.b": 1 } }, ["teams.a.b"])).toEqual({});
	expect(removeKeysFromConfig({ teams: { "a.b.c": 1 } }, ["teams.a.b"])).toEqual({});
	expect(removeKeysFromConfig({
		a: 1,
		b: 2
	}, [])).toEqual({
		a: 1,
		b: 2
	});
	expect(removeKeysFromConfig({
		a: 1,
		b: 2
	}, ["c"])).toEqual({
		a: 1,
		b: 2
	});
	expect(removeKeysFromConfig({
		a: 1,
		b: 2
	}, ["a.b"])).toEqual({
		a: 1,
		b: 2
	});
	expect(removeKeysFromConfig({
		"a.b": 1,
		"c.d": 2,
		"e.f": 3
	}, ["a.b", "e.f"])).toEqual({ "c.d": 2 });
	expect(removeKeysFromConfig({
		a: { b: 1 },
		c: { d: 2 }
	}, ["a.b", "c.d"])).toEqual({});
	expect(removeKeysFromConfig({ a: "string" }, ["a.b"])).toEqual({ a: "string" });
	expect(removeKeysFromConfig({ a: 123 }, ["a.b"])).toEqual({ a: 123 });
	expect(removeKeysFromConfig({ a: null }, ["a.b"])).toEqual({ a: null });
	expect(removeKeysFromConfig({ a: [
		1,
		2,
		3
	] }, ["a.0"])).toEqual({ a: [
		1,
		2,
		3
	] });
});
(void 0)?.test("override(...)", ({ expect }) => {
	expect(override({
		a: 1,
		b: 2,
		"c.d": 3,
		"c.e.f": 4,
		"c.g": 5,
		h: [
			6,
			{ i: 7 },
			8
		],
		k: 123,
		l: void 0
	}, {
		a: 9,
		"c.d": 10,
		"c.e": null,
		"h.0": 11,
		"h.1": { j: 12 },
		k: void 0
	})).toEqual({
		a: 9,
		b: 2,
		"c.d": 10,
		"c.e": null,
		"c.g": 5,
		h: [
			6,
			{ i: 7 },
			8
		],
		"h.0": 11,
		"h.1": { j: 12 },
		k: 123,
		l: void 0
	});
});
var NormalizationError = class extends Error {
	constructor(...args) {
		super(...args);
	}
};
NormalizationError.prototype.name = "NormalizationError";
function isNormalized(c) {
	assertValidConfig(c);
	for (const [key, value] of Object.entries(c)) {
		if (value === void 0) continue;
		if (key.includes(".")) return false;
		if (value === null) return false;
	}
	return true;
}
function assertNormalized(c) {
	assertValidConfig(c);
	if (!isNormalized(c)) throw new ___utils_errors_js.StackAssertionError(`Config is not normalized: ${JSON.stringify(c)}`);
}
function normalize(c, options = {}) {
	assertValidConfig(c);
	const onDotIntoNonObject = options.onDotIntoNonObject ?? "throw";
	const onDotIntoNull = options.onDotIntoNull ?? "like-non-object";
	const countDots = (s) => s.match(/\./g)?.length ?? 0;
	const result = {};
	const keysByDepth = Object.keys(c).sort((a, b) => countDots(a) - countDots(b));
	outer: for (const key of keysByDepth) {
		const keySegmentsWithoutLast = key.split(".");
		const last = keySegmentsWithoutLast.pop() ?? (0, ___utils_errors_js.throwErr)("split returns empty array?");
		const value = (0, ___utils_objects_js.get)(c, key);
		if (value === void 0) continue;
		let current = result;
		for (const keySegment of keySegmentsWithoutLast) {
			if (!(0, ___utils_objects_js.hasAndNotUndefined)(current, keySegment)) switch (onDotIntoNull === "like-non-object" ? onDotIntoNonObject : onDotIntoNull) {
				case "throw": throw new NormalizationError(`Tried to use dot notation to access ${JSON.stringify(key)}, but ${JSON.stringify(keySegment)} doesn't exist on the object (or is null).`);
				case "ignore":
					options.droppedKeys?.push(key);
					continue outer;
				case "empty-object":
					(0, ___utils_objects_js.set)(current, keySegment, {});
					break;
			}
			const value = (0, ___utils_objects_js.get)(current, keySegment);
			if (typeof value !== "object") switch (onDotIntoNonObject) {
				case "throw": throw new NormalizationError(`Tried to use dot notation to access ${JSON.stringify(key)}, but ${JSON.stringify(keySegment)} is not an object.`);
				case "ignore":
					options.droppedKeys?.push(key);
					continue outer;
			}
			current = value;
		}
		setNormalizedValue(current, last, value, options);
	}
	return result;
}
function normalizeValue(value, options) {
	if (value === null) throw new NormalizationError("Tried to normalize a null value");
	if (Array.isArray(value)) return value.map((v) => normalizeValue(v, options));
	if (typeof value === "object") return normalize(value, options);
	return value;
}
function setNormalizedValue(result, key, value, options) {
	if (value === null) {
		if ((0, ___utils_objects_js.hasAndNotUndefined)(result, key)) (0, ___utils_objects_js.deleteKey)(result, key);
	} else (0, ___utils_objects_js.set)(result, key, normalizeValue(value, options));
}
(void 0)?.test("normalize(...)", ({ expect }) => {
	expect(normalize({
		a: 9,
		b: 2,
		c: {},
		"c.d": 10,
		"c.e": null,
		"c.g": 5,
		h: [
			6,
			{ i: 7 },
			8
		],
		"h.0": 11,
		"h.1": { j: 12 },
		k: { l: {} },
		"k.l.m": 13,
		n: void 0
	}, { onDotIntoNonObject: "ignore" })).toEqual({
		a: 9,
		b: 2,
		c: {
			d: 10,
			g: 5
		},
		h: [
			11,
			{ j: 12 },
			8
		],
		k: { l: { m: 13 } }
	});
	expect(() => normalize({ "b.c": 2 }, { onDotIntoNonObject: "throw" })).toThrow(`Tried to use dot notation to access "b.c", but "b" doesn't exist on the object (or is null)`);
	expect(() => normalize({
		b: null,
		"b.c": 2
	}, { onDotIntoNonObject: "throw" })).toThrow(`Tried to use dot notation to access "b.c", but "b" doesn't exist on the object (or is null)`);
	expect(normalize({ "b.c": 2 }, { onDotIntoNonObject: "ignore" })).toEqual({});
	expect(() => normalize({
		b: 1,
		"b.c": 2
	}, { onDotIntoNonObject: "throw" })).toThrow(`Tried to use dot notation to access "b.c", but "b" is not an object`);
	expect(normalize({
		b: 1,
		"b.c": 2
	}, { onDotIntoNonObject: "ignore" })).toEqual({ b: 1 });
});

//#endregion
exports.NormalizationError = NormalizationError;
exports.assertNormalized = assertNormalized;
exports.assertValidConfig = assertValidConfig;
exports.getInvalidConfigReason = getInvalidConfigReason;
exports.isNormalized = isNormalized;
exports.isValidConfig = isValidConfig;
exports.normalize = normalize;
exports.override = override;
exports.removeKeysFromConfig = removeKeysFromConfig;
//# sourceMappingURL=format.js.map