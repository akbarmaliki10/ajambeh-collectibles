import { CollapseObjectUnion, Expand, IsUnion } from "../utils/types.js";
...
declare const branchConfigSchema: yup.Schema<Omit<Omit<{
  ...
  apps: {
    installed: Record<"authentication" | "onboarding" | "teams" | "rbac" | "api-keys" | "payments" | "emails" | "email-api" | "data-vault" | "webhooks" | "tv-mode" | "launch-checklist" | "catalyst" | "neon" | "convex" | "vercel" | "analytics", {
      enabled?: boolean | undefined;
    }>;
  };
  domains: {};
...
declare const projectConfigDefaults: {
  ...
  readonly project: {
    readonly requirePublishableClientKey: false;
  };
};
...
declare const organizationConfigDefaults: {
  ...
  readonly emails: {
    readonly server: {
      ...
      readonly managedSubdomain: undefined;
      readonly managedSenderLocalPart: undefined;
    };
  ...
  readonly dbSync: {
    readonly externalDatabases: (key: string) => {
      type: undefined;
  ...
  readonly analytics: {
    readonly queryFolders: (key: string) => {
      displayName: string;
      sortOrder: number;
  ...
  readonly customDashboards: (key: string) => {
    displayName: string;
    tsxSource: string;
  };
};
...
type _ValidatedToHaveNoConfigOverrideErrorsImpl<T> = IsUnion<T & object> extends true ? _ValidatedToHaveNoConfigOverrideErrorsImpl<CollapseObjectUnion<T & object> | Exclude<T, object>> : T extends object ? (T extends any[] ? T : { [K in keyof T]+?: _ValidatedToHaveNoConfigOverrideErrorsImpl<T[K]> }) : T;
