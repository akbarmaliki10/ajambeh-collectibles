...
function getKmsClient() {
	...
	return new KMSClient({
		...
		credentials: roleArn ? awsCredentialsProvider({ roleArn }) : {
...
async function genDEK() {
	...
	const out = await getKmsClient().send(new GenerateDataKeyCommand({
		KeyId: kekId,
		KeySpec: "AES_256"
	}));
...
async function unwrapDEK(edk_b64) {
	...
	const out = await getKmsClient().send(new DecryptCommand({ CiphertextBlob: edkBytes }));
...
async function encryptWithKms(value) {
	...
	try {
		...
		return {
			edkBase64: encodeBase64(edkBytes),
