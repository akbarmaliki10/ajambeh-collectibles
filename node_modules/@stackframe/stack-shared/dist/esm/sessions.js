...
import { runAsynchronously, wait } from "./utils/promises.js";
...
var AccessToken = class AccessToken {
	...
	constructor(token) {
		this.token = token;
		if (token === "undefined") throw new StackAssertionError("Access token is the string 'undefined'; it's unlikely this is the correct value. They're supposed to be unguessable!");
	}
	get payload() {
...
var RefreshToken = class {
	constructor(token) {
		this.token = token;
		if (token === "undefined") throw new StackAssertionError("Refresh token is the string 'undefined'; it's unlikely this is the correct value. They're supposed to be unguessable!");
	}
};
/**
	...
	*/
	async getOrFetchLikelyValidTokens(minMillisUntilExpiration, maxMillisSinceIssued) {
		...
		if (!accessToken) {
			...
			if (expiresInMillis !== void 0 && expiresInMillis < minMillisUntilExpiration) throw new StackAssertionError(`Required access token expiry ${minMillisUntilExpiration}ms is too long; access tokens are too short when they're generated (${expiresInMillis}ms)`);
			if (maxMillisSinceIssued !== null && issuedMillisAgo !== void 0 && issuedMillisAgo > maxMillisSinceIssued) throw new StackAssertionError(`Required access token issuance ${maxMillisSinceIssued}ms is too short; access token issuance is too slow (${issuedMillisAgo}ms)`);
			return newTokens;
		}
		return {
			accessToken,
