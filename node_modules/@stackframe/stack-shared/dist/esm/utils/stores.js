...
const storeLock = new ReadWriteLock();
var AsyncStore = class AsyncStore {
	...
	get() {
		if (this.isRejected()) return AsyncResult.error(this._rejectionError);
		else if (this.isAvailable()) return AsyncResult.ok(this._mostRecentOkValue);
		else return AsyncResult.pending();
	}
	getOrWait() {
		const uuid = generateUuid();
		if (this.isRejected()) return rejected(this._rejectionError);
		else if (this.isAvailable()) return resolved(this._mostRecentOkValue);
...
//#endregion
export { AsyncStore, Store, storeLock };
