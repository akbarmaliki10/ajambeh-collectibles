...
async function getJwtInfo(options) {
	try {
		if (typeof options.jwt !== "string") return Result.error({
			error: "JWT input is not a string!",
			stringifiedInput: nicify(options.jwt)
		});
		if (!options.jwt.startsWith("ey")) return Result.error({
		...
		return Result.error({ exception: errorToNiceString(e) });
	}
}
...
async function verifyJWT(options) {
	...
	if (!audience || typeof audience !== "string") throw new JOSEError("Invalid JWT audience");
...
async function getPrivateJwkFromDerivedSecret(derivedSecret, kid) {
	...
	const publicKey = new elliptic.ec("p256").keyFromPrivate(priv).getPublic();
	return {
		kty: "EC",
		crv: "P-256",
		alg: "ES256",
...
async function getPublicJwkSet(privateJwks) {
	return { keys: privateJwks.map((jwk) => pick(jwk, [
		"kty",
		"alg",
		"crv",
		"x",
...
function oldGetPerAudienceSecret(options) {
	if (options.audience === "kid") throw new StackAssertionError("You cannot use the 'kid' audience for a per-audience secret, see comment below in jwt.tsx");
	return jose.base64url.encode(crypto.createHash("sha256").update(JSON.stringify([getStackServerSecret(), options.audience])).digest());
}
...
//#endregion
export { getJwtInfo, getPrivateJwks, getPublicJwkSet, oldGetKid, signJWT, verifyJWT };
