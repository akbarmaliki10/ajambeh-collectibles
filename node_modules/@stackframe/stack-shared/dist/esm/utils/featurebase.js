...
//#region src/utils/featurebase.tsx
/**
...
async function findFeaturebaseUserById(stackAuthUserId, apiKey) {
	try {
		...
		if (!response.ok) throw new StackAssertionError(`Failed to find Featurebase user by ID: ${response.statusText}`);
		const data = await response.json();
		const user = data.user;
		if (!user) throw new StackAssertionError(`Featurebase API returned success but no user data for ID: ${stackAuthUserId}`, { data });
		return {
		...
		if (error instanceof StackAssertionError) throw error;
...
async function findFeaturebaseUserByEmail(email, apiKey) {
	try {
		...
		if (response.status === 404) return null;
		if (!response.ok) throw new StackAssertionError(`Failed to find Featurebase user by email: ${response.statusText}`);
		const data = await response.json();
		const user = data.user;
		if (!user) throw new StackAssertionError(`Featurebase API returned success but no user data for email: ${email}`, { data });
		return {
...
async function getOrCreateFeaturebaseUser(stackAuthUser, options) {
	...
	if (existingById) {
		...
		try {
			const updates = {};
			if (stackAuthUser.displayName && stackAuthUser.displayName !== existingById.name) updates.name = stackAuthUser.displayName;
			if (stackAuthUser.profileImageUrl && stackAuthUser.profileImageUrl !== existingById.profilePicture) updates.profilePicture = stackAuthUser.profileImageUrl;
...
//#endregion
export { getOrCreateFeaturebaseUser };
