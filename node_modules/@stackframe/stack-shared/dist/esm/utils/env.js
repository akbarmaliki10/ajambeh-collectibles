...
function getEnvVariable(name, defaultValue) {
	if (isBrowserLike()) throw new Error(deindent`
    ...
    `);
	if (name === "NEXT_RUNTIME") throw new Error(deindent`
	...
	else throwErr(`Missing environment variable: ${name}`);
	return value;
}
function getEnvBoolean(name) {
	...
	else throw new StackAssertionError(`Environment variable ${name} must be either "true" or "false": found ${JSON.stringify(value)}`);
}
...
//#endregion
export { getEnvBoolean, getEnvVariable, getNextRuntime, getNodeEnvironment, isBrowserLike };
