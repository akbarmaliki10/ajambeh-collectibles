...
//#region src/utils/strings.nicify.test.ts
describe("nicify", () => {
	...
	describe("built-in objects", () => {
		...
		test("TypedArrays", () => {
			expect(nicify(new Uint8Array([
				1,
				2,
				3
			]))).toBe("Uint8Array([1,2,3])");
			expect(nicify(new Int32Array([
				1,
				2,
				3
			]))).toBe("Int32Array([1,2,3])");
		});
		test("Error objects", () => {
			expect(nicify({ error: /* @__PURE__ */ new Error("test error") })).toMatch(new RegExp(deindent`
        ^\{
      ...
      `));
		...
		test("Error objects with cause and an extra property", () => {
			const error = new Error("test error", { cause: /* @__PURE__ */ new Error("cause") });
			error.extra = "something";
			expect(nicify(error, { lineIndent: "--" })).toMatch(new RegExp(deindent`
        ^Error: test error
        --Stack:
        ----at (.|\\n)+
	...
	describe("configuration options", () => {
		test("maxDepth", () => {
			expect(nicify({ a: { b: { c: { d: { e: 1 } } } } }, { maxDepth: 2 })).toBe("{ \"a\": { \"b\": { ... } } }");
		});
		test("lineIndent", () => {
			expect(nicify({
				a: 1,
				b: 2
			}, { lineIndent: "    " })).toBe(deindent`
        {
            "a": 1,
            "b": 2,
        }
      `);
		...
		test("hideFields", () => {
			expect(nicify({
				...
				b: 2,
				secret: "hidden"
			}, { hideFields: ["secret"] })).toBe(deindent`
        {
          "a": 1,
          "b": 2,
	...
	describe("custom overrides", () => {
		test("override with custom type", () => {
			expect(nicify({ type: "special" }, { overrides: ((value) => {
				if (typeof value === "object" && value && "type" in value && value.type === "special") return "SPECIAL";
				return null;
			}) })).toBe("SPECIAL");
		});
	});
