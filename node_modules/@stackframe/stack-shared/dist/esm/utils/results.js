...
async function retry(fn, totalAttempts, { exponentialDelayBase = 1e3 } = {}) {
	...
	for (let i = 0; i < totalAttempts; i++) {
		...
		if (res.status === "ok") return Object.assign(Result.ok(res.data), { attempts: i + 1 });
			...
			if (i < totalAttempts - 1) await wait((Math.random() + .5) * exponentialDelayBase * 2 ** i);
		}
	}
...
(void 0)?.test("retry", async ({ expect }) => {
	...
	expect(await retry(successFn, 3, { exponentialDelayBase: 0 })).toEqual({
		status: "ok",
		data: "success",
		attempts: 1
	});
	...
	expect(await retry(eventualSuccessFn, 3, { exponentialDelayBase: 0 })).toEqual({
		status: "ok",
