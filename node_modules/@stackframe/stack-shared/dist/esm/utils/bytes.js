...
function getBase32CharacterFromIndex(index) {
	if (index < 0 || index >= 32) throw new StackAssertionError(`Invalid base32 index: ${index}`);
...
function getBase32IndexFromCharacter(character) {
	if (character.length !== 1) throw new StackAssertionError(`Invalid base32 character: ${character}`);
	const index = crockfordAlphabet.indexOf(character.toUpperCase());
	if (index === -1) throw new StackAssertionError(`Invalid base32 character: ${character}`);
	return index;
}
...
function encodeBase32(input) {
	...
	if (!isBase32(output)) throw new StackAssertionError("Invalid base32 output; this should never happen");
	return output;
}
...
function decodeBase32(input) {
	if (!isBase32(input)) throw new StackAssertionError("Invalid base32 string");
...
(void 0)?.test("encodeBase64/decodeBase64", ({ expect }) => {
	...
	for (const [i, { input, expected }] of testCases.entries()) {
		...
		if (encoded !== expected) throw new StackAssertionError(`encodeBase64 test case ${i} failed`);
		if (decodeBase64(encoded).some((b, i) => b !== input[i])) throw new StackAssertionError(`decodeBase64 test case ${i} failed`);
	}
...
function decodeBase64Url(input) {
	if (!isBase64Url(input)) throw new StackAssertionError("Invalid base64url string");
...
function decodeBase64OrBase64Url(input) {
	...
	else throw new StackAssertionError("Invalid base64 or base64url string");
}
