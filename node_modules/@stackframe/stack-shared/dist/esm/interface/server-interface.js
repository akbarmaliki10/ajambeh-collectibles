...
var StackServerInterface = class extends StackClientInterface {
	...
	async listServerUsers(options) {
		const searchParams = new URLSearchParams(filterUndefined({
			...
			...options.orderBy ? { order_by: { signedUpAt: "signed_up_at" }[options.orderBy] } : {},
	...
	async listServerTeams(options) {
		return (await (await this.sendServerRequest(`/teams?${new URLSearchParams(filterUndefined({ user_id: options?.userId }))}`, {}, null)).json()).items;
	}
	...
	*/
	...
	async updateItemQuantity(options, data) {
		...
		} else throw new StackAssertionError("updateItemQuantity requires one of userId, teamId, or customCustomerId");
	...
	async grantProduct(options) {
		if (!options.productId && !options.product) throw new StackAssertionError("grantProduct requires either productId or product");
		if (options.productId && options.product) throw new StackAssertionError("grantProduct should not receive both productId and product");
	...
	async getDataVaultStoreValue(secret, storeId, key) {
		...
		else throw new StackAssertionError("Unexpected uncaught error", { cause: response.error });
