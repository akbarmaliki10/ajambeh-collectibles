import { customerTypeSchema, inlineProductSchema, moneyAmountSchema, yupArray, yupBoolean, yupNumber, yupObject, yupString, yupUnion } from "../../schema-fields.js";
...
//#region src/interface/crud/transactions.ts
...
const chargedAmountSchema = yupObject({ ...typedFromEntries(SUPPORTED_CURRENCIES.map((currency) => [currency.code, moneyAmountSchema(currency).optional()])) }).noUnknown(true).test("at-least-one-currency", "charged_amount must include at least one currency amount", (value) => {
	return Object.values(value).some((amount) => typeof amount === "string");
}).defined();
const netAmountSchema = yupObject({ USD: moneyAmountSchema(USD_CURRENCY).defined() }).noUnknown(true).defined();
...
}).test("exclusive-reference", "subscription_id and one_time_purchase_id cannot both be set", (value, context) => {
	if (value.subscription_id != null && value.one_time_purchase_id != null) return context.createError({ message: "subscription_id and one_time_purchase_id cannot both be set" });
	return true;
}).defined();
...
const transactionEntrySchema = yupUnion(transactionEntryMoneyTransferSchema, transactionEntryItemQuantityChangeSchema, transactionEntryProductGrantSchema, transactionEntryProductRevocationSchema, transactionEntryProductRevocationReversalSchema).defined();
...
const transactionSchema = yupObject({
	...
	adjusted_by: yupArray(yupObject({
...
//#endregion
export { TRANSACTION_TYPES, transactionEntrySchema, transactionSchema };
