...
import { yupArray, yupObject, yupString } from "../../schema-fields.js";

//#region src/interface/crud/projects.ts
const teamPermissionSchema = yupObject({ id: yupString().defined() }).defined();
const oauthProviderReadSchema = yupObject({
	...
	client_id: schemaFields.yupDefinedAndNonEmptyWhen(schemaFields.oauthClientIdSchema, { type: "standard" }),
	client_secret: schemaFields.yupDefinedAndNonEmptyWhen(schemaFields.oauthClientSecretSchema, { type: "standard" }),
...
const enabledOAuthProviderSchema = yupObject({ id: schemaFields.oauthIdSchema.defined() });
const emailConfigSchema = yupObject({
	...
	sender_name: schemaFields.yupDefinedAndNonEmptyWhen(schemaFields.emailSenderNameSchema, { type: "standard" }),
	sender_email: schemaFields.yupDefinedAndNonEmptyWhen(schemaFields.emailSenderEmailSchema, { type: "standard" })
});
const emailConfigWithoutPasswordSchema = emailConfigSchema.pick([
	"type",
	"host",
	"port",
	"username",
...
const domainSchema = yupObject({
	domain: schemaFields.urlSchema.defined().matches(/^https?:\/\//, "URL must start with http:// or https://").meta({ openapiField: {
		description: "URL. Must start with http:// or https://",
		exampleValue: "https://example.com"
	} }),
	handler_path: schemaFields.handlerPathSchema.defined()
});
const projectsCrudAdminReadSchema = yupObject({
...
const projectsCrudAdminUpdateSchema = yupObject({
	...
	config: yupObject({
		...
		oauth_providers: yupArray(oauthProviderWriteSchema.defined()).optional().default(void 0),
...
const projectsCrudAdminCreateSchema = projectsCrudAdminUpdateSchema.concat(yupObject({
	display_name: schemaFields.projectDisplayNameSchema.defined(),
	owner_team_id: schemaFields.yupString().uuid().defined()
}).defined());
const projectsCrudAdminDeleteSchema = schemaFields.yupMixed();
const clientProjectsCrud = createCrud({
...
const adminUserProjectsCrud = createCrud({
	clientReadSchema: projectsCrudAdminReadSchema,
	clientCreateSchema: projectsCrudAdminCreateSchema,
	docs: {
		clientList: { hidden: true },
		clientCreate: { hidden: true }
	}
});

//#endregion
export { adminUserProjectsCrud, clientProjectsCrud, emailConfigSchema, emailConfigWithoutPasswordSchema, projectsCrud, projectsCrudAdminCreateSchema, projectsCrudAdminDeleteSchema, projectsCrudAdminReadSchema, projectsCrudAdminUpdateSchema, projectsCrudClientReadSchema };
