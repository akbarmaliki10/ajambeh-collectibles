...
const botChallengeKnownErrors = [KnownErrors.BotChallengeRequired, KnownErrors.BotChallengeFailed];
...
function getBotChallengeRequestFields(botChallenge, context) {
	if (botChallenge?.unavailable) {
		if (botChallenge.token != null || botChallenge.phase != null) throw new StackAssertionError(`${context} bot challenge unavailability cannot be combined with a token or phase.`);
	...
	if (challengeToken == null) {
		if (botChallenge?.phase != null) throw new StackAssertionError(`${context} bot challenge phase options require a token.`);
		return {};
	}
...
var StackClientInterface = class {
	...
	async _networkRetry(cb, session, requestType) {
		const retriedResult = await Result.retry(cb, 5, { exponentialDelayBase: 1e3 });
		if (retriedResult.status === "error") {
			if (globalVar.navigator && globalVar.navigator.onLine === false) throw new Error("You are offline. Please check your internet connection and try again. (window.navigator.onLine is false)", { cause: retriedResult.error });
	...
	async fetchNewAccessToken(refreshToken) {
		...
		const response = await this._networkRetryException(async () => {
			...
			if (response.status === "error") {
				...
				if (KnownErrors.RefreshTokenError.isInstance(error)) return null;
				throw error;
			}
			if (!response.data.ok) {
		...
		try {
			...
			if (e instanceof ResponseBodyError) throw new StackAssertionError("ResponseBodyError when processing refresh token response", {
				cause: e.cause,
				code: e.code,
				error: e.error
			});
		...
		if (!result.access_token) throw new StackAssertionError("Access token not found in token endpoint response, this is weird!");
	...
	async sendClientRequest(path, requestOptions, session, requestType = "client", apiUrlOverride) {
		session ??= this.createSession({ refreshToken: null });
		return await this._networkRetry(() => this.sendClientRequestInner(path, requestOptions, session, requestType, apiUrlOverride), session, requestType);
	}
	...
	async sendClientRequestInner(path, options, session, requestType, apiUrlOverride) {
		/**
		...
		*/
		...
		const params = {
			..."WebSocketPair" in globalVar ? {} : { credentials: "omit" },
			...options,
			headers: {
				...
				...tokenObj ? { "X-Stack-Access-Token": tokenObj.accessToken.token } : {},
				...tokenObj?.refreshToken ? { "X-Stack-Refresh-Token": tokenObj.refreshToken.token } : {},
				"X-Stack-Allow-Anonymous-User": "true",
				..."publishableClientKey" in this.options && this.options.publishableClientKey ? { "X-Stack-Publishable-Client-Key": this.options.publishableClientKey } : {},
				...adminTokenObj ? { "X-Stack-Admin-Access-Token": adminTokenObj.accessToken.token } : {},
		...
		try {
			...
			if (e instanceof TypeError) if (HTTP_METHODS[params.method ?? "GET"].idempotent) return Result.error(e);
		...
		if (processedRes.status === "error") {
			if (KnownErrors.InvalidAccessToken.isInstance(processedRes.error)) {
				if (!tokenObj) throw new StackAssertionError("Received invalid access token, but session is not logged in", {
					tokenObj,
					processedRes
				});
			...
			if (adminSession && (KnownErrors.InvalidAdminAccessToken.isInstance(processedRes.error) || KnownErrors.ApiKeyNotFound.isInstance(processedRes.error))) {
				if (!adminTokenObj) throw new StackAssertionError("Received invalid admin access token, but admin session is not logged in", {
					adminTokenObj,
					processedRes
				});
		...
		if (res.ok) return Result.ok(res);
			...
			if (retryAfter !== null) {
				...
				return Result.error(/* @__PURE__ */ new Error(`Rate limited, retrying after ${retryAfter} seconds`));
			}
			console.log(`Rate limited while sending request to ${url}, no retry-after header received. Retrying...`);
			return Result.error(/* @__PURE__ */ new Error("Rate limited, no retry-after header received"));
		} else {
			const error = await res.text();
			const errorObj = new StackAssertionError(`Failed to send request to ${url}: ${res.status} ${error}`, {
				request: params,
				res,
				path
			});
			if (res.status === 508 && error.includes("INFINITE_LOOP_DETECTED")) return Result.error(errorObj);
			throw errorObj;
		}
	}
	async _processResponse(rawRes) {
		...
		if (res.headers.has("x-stack-known-error")) {
			...
			if (res.headers.get("x-stack-known-error") !== errorJson.code) throw new StackAssertionError("Mismatch between x-stack-known-error header and error code in body; the server's response is invalid");
	...
	async checkFeatureSupport(options) {
		throw new StackAssertionError(await (await this.sendClientRequest("/check-feature-support", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
	...
	async sendForgotPasswordEmail(email, callbackUrl) {
		...
		if (res.status === "error") return Result.error(res.error);
		else return Result.ok(void 0);
	}
	...
	async sendMagicLinkEmail(email, callbackUrl, botChallenge) {
		...
		}, null, [KnownErrors.RedirectUrlNotWhitelisted, ...botChallengeKnownErrors]);
		if (res.status === "error") return Result.error(res.error);
		else return Result.ok(await res.data.json());
	}
	...
	async signInWithMagicLink(code, session) {
		const res = await this.sendClientRequestAndCatchKnownError("/auth/otp/sign-in", {
			...
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ code })
		}, session, [KnownErrors.VerificationCodeError]);
		if (res.status === "error") return Result.error(res.error);
		const result = await res.data.json();
		return Result.ok({
			accessToken: result.access_token,
	...
	async signInWithPasskey(body, session) {
		...
		}, session, [KnownErrors.PasskeyAuthenticationFailed]);
		if (res.status === "error") return Result.error(res.error);
		const result = await res.data.json();
		return Result.ok({
			accessToken: result.access_token,
	...
	async authorizeOAuth(options) {
		if (typeof window === "undefined") throw new StackAssertionError("authorizeOAuth can currently only be called in a browser environment");
		await this.options.prepareRequest?.();
		...
		if (processedResponse.status === "error") {
			if (isBotChallengeKnownError(processedResponse.error)) return Result.error(processedResponse.error);
			throw processedResponse.error;
		}
		if (processedResponse.data.status !== 200) throw new StackAssertionError(`OAuth authorize returned an unexpected status: ${processedResponse.data.status}`);
		const body = await processedResponse.data.json();
		if (body == null || typeof body !== "object" || Array.isArray(body)) throw new StackAssertionError("OAuth authorize response body must be an object", { body });
		const location = body.location;
		if (typeof location !== "string") throw new StackAssertionError("OAuth authorize response is missing a redirect location", { body });
	...
	async callOAuthCallback(options) {
		...
		try {
			...
			if (e instanceof AuthorizationResponseError) throw new StackAssertionError("Authorization response error when validating outer OAuth response", {
		...
		try {
			...
			if (e instanceof ResponseBodyError) {
				if (e.cause.code === "MULTI_FACTOR_AUTHENTICATION_REQUIRED") throw new KnownErrors.MultiFactorAuthenticationRequired(e.cause.details.attempt_code);
	...
	async signOut(session) {
		if (await session.getOrFetchLikelyValidTokens(2e4, null)) {
			...
			if (resOrError.status === "error") if (KnownErrors.RefreshTokenError.isInstance(resOrError.error)) {} else throw new StackAssertionError("Unexpected error", { error: resOrError.error });
		}
		session.markInvalid();
	}
	async getClientUserByToken(session) {
		...
		else throw new StackAssertionError("Unexpected uncaught error", { cause: responseOrError.error });
	...
	async acceptTeamInvitationById(invitationId, session) {
		await this.sendClientRequest(urlString`/team-invitations/${invitationId}/accept` + "?" + new URLSearchParams({ user_id: "me" }), { method: "POST" }, session);
	}
	...
	async listTeamMemberProfiles(options, session) {
		return (await (await this.sendClientRequest("/team-member-profiles?" + new URLSearchParams(filterUndefined({
			team_id: options.teamId,
	...
	*/
	...
	async _getApiKeyRequestInfo(options) {
		if ("user_id" in options && "team_id" in options) throw new StackAssertionError("Cannot specify both user_id and team_id in _getApiKeyRequestInfo");
		return {
	...
	async getItem(options, session, requestType = "client") {
		...
		} else throw new StackAssertionError("getItem requires one of userId, teamId, or customCustomerId");
		return await (await (requestType === "client" ? this.sendClientRequest : this.sendServerRequest).bind(this)(urlString`/payments/items/${customerType}/${customerId}/${options.itemId}`, {}, session, requestType)).json();
	}
	...
	async cancelSubscription(options, session) {
		const queryParams = new URLSearchParams(filterUndefined({ subscription_id: options.subscription_id }));
	...
	async switchSubscription(options, session) {
		await this.sendClientRequest(urlString`/payments/products/${options.customer_type}/${options.customer_id}/switch`, {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({
	...
	async getCustomerBilling(customerType, customerId, session) {
		return await (await this.sendClientRequest(urlString`/payments/billing/${customerType}/${customerId}`, {}, session)).json();
	}
	async createCustomerPaymentMethodSetupIntent(customerType, customerId, session) {
		return await (await this.sendClientRequest(urlString`/payments/payment-method/${customerType}/${customerId}/setup-intent`, {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({})
		}, session)).json();
	...
	async setDefaultCustomerPaymentMethodFromSetupIntent(customerType, customerId, setupIntentId, session) {
		return await (await this.sendClientRequest(urlString`/payments/payment-method/${customerType}/${customerId}/set-default`, {
			method: "POST",
			headers: { "content-type": "application/json" },
	...
	async transferProject(internalProjectSession, projectIdToTransfer, newTeamId) {
		if (this.options.projectId !== "internal") throw new StackAssertionError("StackClientInterface.transferProject() is only available for internal projects (please specify the project ID in the constructor)");
