...
var StackAdminInterface = class extends StackServerInterface {
	...
	async getProject() {
		return await (await this.sendAdminRequest("/internal/projects/current", { method: "GET" }, null)).json();
	}
	...
	async deleteEmailDraft(id) {
		await this.sendAdminRequest(`/internal/email-drafts/${id}`, { method: "DELETE" }, null);
	}
	...
	async deleteProject() {
		await this.sendAdminRequest("/internal/projects/current", { method: "DELETE" }, null);
	}
	async getMetrics(includeAnonymous = false) {
		const params = new URLSearchParams();
		if (includeAnonymous) params.append("include_anonymous", "true");
		const queryString = params.toString();
		return await (await this.sendAdminRequest(`/internal/metrics${queryString ? `?${queryString}` : ""}`, { method: "GET" }, null)).json();
	}
	...
	async listSentEmails() {
		return await (await this.sendAdminRequest("/internal/emails", { method: "GET" }, null)).json();
	}
	...
	async listManagedEmailDomains() {
		return await (await this.sendAdminRequest("/internal/emails/managed-onboarding/list", { method: "GET" }, null)).json();
	}
	...
	async listChatMessages(threadId) {
		return await (await this.sendAdminRequest(`/internal/ai-chat/${threadId}`, { method: "GET" }, null)).json();
	}
	...
	async rewriteTemplateSourceWithAI(templateTsxSource) {
		return await (await this.sendAdminRequest(`/internal/rewrite-template-source`, {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({ template_tsx_source: templateTsxSource })
		}, null)).json();
	}
	...
	async getEmailTheme(id) {
		return await (await this.sendAdminRequest(`/internal/email-themes/${id}`, { method: "GET" }, null)).json();
	}
	...
	async deleteEmailTheme(id) {
		await this.sendAdminRequest(`/internal/email-themes/${id}`, { method: "DELETE" }, null);
	}
	...
	async getConfig() {
		return await (await this.sendAdminRequest(`/internal/config`, { method: "GET" }, null)).json();
	}
	async getConfigOverride(level) {
		return await (await this.sendAdminRequest(`/internal/config/override/${level}`, { method: "GET" }, null)).json();
	}
	...
	async getPushedConfigSource() {
		return (await (await this.sendAdminRequest(`/internal/config/source`, { method: "GET" }, null)).json()).source;
	}
	...
	async deleteEmailTemplate(id) {
		await this.sendAdminRequest(`/internal/email-templates/${id}`, { method: "DELETE" }, null);
	}
	async setupPayments() {
	...
	async getStripeAccountInfo() {
		const response = await this.sendAdminRequestAndCatchKnownError("/internal/payments/stripe/account-info", {}, null, [KnownErrors.StripeAccountInfoNotFound]);
		if (response.status === "error") return null;
		return await response.data.json();
	}
	async getPaymentMethodConfigs() {
		const response = await this.sendAdminRequestAndCatchKnownError("/internal/payments/method-configs", { method: "GET" }, null, [KnownErrors.StripeAccountInfoNotFound]);
		if (response.status === "error") return null;
	...
	async listTransactions(params) {
		...
		const json = await (await this.sendAdminRequest(`/internal/payments/transactions${qs.size ? `?${qs.toString()}` : ""}`, { method: "GET" }, null)).json();
		return {
	...
	async listSessionReplays(params) {
		...
		return await (await this.sendAdminRequest(`/internal/session-replays${qs.size ? `?${qs.toString()}` : ""}`, { method: "GET" }, null)).json();
	}
	async listSessionReplayChunks(sessionReplayId, params) {
		const qs = new URLSearchParams();
		if (params?.cursor) qs.set("cursor", params.cursor);
		if (typeof params?.limit === "number") qs.set("limit", String(params.limit));
		return await (await this.sendAdminRequest(`/internal/session-replays/${encodeURIComponent(sessionReplayId)}/chunks${qs.size ? `?${qs.toString()}` : ""}`, { method: "GET" }, null)).json();
	}
	async getSessionReplayChunkEvents(sessionReplayId, chunkId) {
		return await (await this.sendAdminRequest(`/internal/session-replays/${encodeURIComponent(sessionReplayId)}/chunks/${encodeURIComponent(chunkId)}/events`, { method: "GET" }, null)).json();
	}
	async getSessionReplayEvents(sessionReplayId, options) {
		...
		return await (await this.sendAdminRequest(`/internal/session-replays/${encodeURIComponent(sessionReplayId)}/events${qs.size ? `?${qs.toString()}` : ""}`, { method: "GET" }, null)).json();
	}
	async refundTransaction(options) {
		return await (await this.sendAdminRequest("/internal/payments/transactions/refund", {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({
				type: options.type,
	...
	async listOutboxEmails(options) {
		...
		return await (await this.sendServerRequest(`/emails/outbox${qs.size ? `?${qs.toString()}` : ""}`, { method: "GET" }, null)).json();
	}
	async getOutboxEmail(id) {
		return await (await this.sendServerRequest(`/emails/outbox/${id}`, { method: "GET" }, null)).json();
	}
