//#region src/payments/payment-methods.ts
/**
...
function getPaymentMethodName(methodId) {
	if (!isKnownPaymentMethod(methodId)) return methodId;
...
function getPaymentMethodCategory(methodId) {
	if (!isKnownPaymentMethod(methodId)) return;
...
*/
const PAYMENT_METHOD_DEPENDENCIES = Object.fromEntries(Object.entries(PAYMENT_METHODS).filter(([, config]) => config.dependencies && config.dependencies.length > 0).map(([id, config]) => [id, config.dependencies]));
/**
...
function getPaymentMethodsByCategory() {
	...
	for (const [id, config] of Object.entries(PAYMENT_METHODS)) result[config.category].push(id);
	return result;
}

//#endregion
export { PAYMENT_CATEGORIES, PAYMENT_METHODS, PAYMENT_METHOD_DEPENDENCIES, getAllPaymentMethodIds, getAllPaymentMethodNames, getPaymentMethodCategory, getPaymentMethodDependencies, getPaymentMethodName, getPaymentMethodsByCategory, isKnownPaymentMethod };
