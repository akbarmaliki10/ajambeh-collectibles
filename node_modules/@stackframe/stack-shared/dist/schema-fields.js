...
yup.addMethod(yup.Schema, "hasNested", function(path) {
	if (!path.match(/^[a-zA-Z0-9_$:-]*$/)) throw new __utils_errors_js.StackAssertionError(`yupSchema.hasNested can currently only be used with alphanumeric keys, underscores, dollar signs, colons, and hyphens. Fix this in the future. Provided key: ${JSON.stringify(path)}`);
...
yup.addMethod(yup.Schema, "getNested", function(path) {
	if (!path.match(/^[a-zA-Z0-9_$:-]*$/)) throw new __utils_errors_js.StackAssertionError(`yupSchema.getNested can currently only be used with alphanumeric keys, underscores, dollar signs, colons, and hyphens. Fix this in the future. Provided key: ${JSON.stringify(path)}`);
	if (!this.hasNested(path)) throw new __utils_errors_js.StackAssertionError(`Tried to call yupSchema.getNested, but key is not present in the schema. Provided key: ${path}`, {
		path,
		schema: this
	});
	const schemaInfo = this.meta()?.stackSchemaInfo;
	...
	try {
		...
		if (error instanceof ReplaceFieldWithOwnUserId) {
			...
			while (pathRemaining.length > 0) if (pathRemaining.startsWith("[")) {
				...
				if (index < 0) throw new __utils_errors_js.StackAssertionError("Invalid path");
			...
			for (const field of fieldPath.slice(0, -1)) {
				if (!Object.prototype.hasOwnProperty.call(it, field)) throw new __utils_errors_js.StackAssertionError(`Segment ${field} of path ${error.path} not found in object`);
				it = it[field];
			}
...
function ensureObjectSchema(schema) {
	if (!(schema instanceof yup.ObjectSchema)) throw new __utils_errors_js.StackAssertionError(`assertObjectSchema: schema is not an ObjectSchema: ${schema.describe().type}`);
	return schema;
}
...
const priceOrIncludeByDefaultSchema = yupUnion(yupString().oneOf(["include-by-default"]).meta({ openapiField: {
...
const userIdOrMeSchema = yupString().uuid().transform((v) => {
	...
	if (!("stackAllowUserIdMe" in (context.options.context ?? {}))) throw new __utils_errors_js.StackAssertionError("userIdOrMeSchema is not allowed in this context. Make sure you're using yupValidate from schema-fields.ts to validate, instead of schema.validate(...).");
	if (!context.options.context?.stackAllowUserIdMe) throw new __utils_errors_js.StackAssertionError("userIdOrMeSchema is not allowed in this context. Make sure you're passing in the currentUserId option in yupValidate.");
...
const neonAuthorizationHeaderSchema = basicAuthorizationHeaderSchema.test("is-authorization-header", "Invalid client_id:client_secret values; did you use the correct values for the integration?", (value) => {
	if (!value) return true;
	const [clientId, clientSecret] = (0, __utils_http_js.decodeBasicAuthorizationHeader)(value) ?? (0, __utils_errors_js.throwErr)(`Authz header invalid? This should've been validated by basicAuthorizationHeaderSchema: ${value}`);
...
//#endregion
Object.defineProperty(exports, 'ISO_3166_ALPHA_2_COUNTRY_CODES', {
  enumerable: true,
  get: function () {
    return __utils_country_codes_js.ISO_3166_ALPHA_2_COUNTRY_CODES;
  }
});
...
exports.priceOrIncludeByDefaultSchema = priceOrIncludeByDefaultSchema;
...
Object.defineProperty(exports, 'validCountryCodeSet', {
  enumerable: true,
  get: function () {
    return __utils_country_codes_js.validCountryCodeSet;
  }
});
