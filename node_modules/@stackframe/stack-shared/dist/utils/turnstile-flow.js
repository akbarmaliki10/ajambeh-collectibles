...
async function executeTurnstileInvisible(siteKey, action) {
	...
	if (!api) throw new __errors_js.StackAssertionError("Turnstile API not available after loadTurnstileScript() resolved");
	...
	try {
		...
		if (widgetId != null) try {
			...
			(0, __errors_js.captureError)("turnstile-widget-remove", e instanceof Error ? e : new __errors_js.StackAssertionError("Non-Error thrown during Turnstile widget removal", { cause: e }));
		}
		container.remove();
	}
}
...
function showTurnstileVisibleChallenge(siteKey, action) {
	...
	return new Promise((resolve, reject) => {
		...
		(0, __turnstile_browser_js.loadTurnstileScript)().then(() => {
			...
			if (!api) {
				cleanup();
				reject(new __errors_js.StackAssertionError("Turnstile API not available after loadTurnstileScript() resolved"));
				return;
			}
...
async function withBotChallengeFlow(options) {
	...
	try {
		...
		try {
			...
			(0, __errors_js.captureError)("turnstile-flow-all-challenges-failed", e instanceof Error ? e : new __errors_js.StackAssertionError("Non-Error thrown during Turnstile challenge", { cause: e }));
	...
	try {
		...
		if (e instanceof BotChallengeUserCancelledError) throw e;
		(0, __errors_js.captureError)("turnstile-flow-visible-challenge-failed", e instanceof Error ? e : new __errors_js.StackAssertionError("Non-Error thrown during visible Turnstile challenge", { cause: e }));
