...
function throwingProxy(error) {
	const doThrow = () => {
		if (typeof error === "string") (0, __errors_js.throwErr)(error);
		else (0, __errors_js.throwErr)(error);
	};
	return new Proxy(() => {}, {
		get: doThrow,
		set: doThrow,
	...
	return new Proxy(toLog, {
		get(target, prop, receiver) {
			...
			if (typeof orig === "function") return function(...args) {
				const success = (v, isPromise) => console.debug(`logged(...): Called ${name}.${String(prop)}(${args.map((a) => (0, __strings_js.nicify)(a)).join(", ")}) => ${isPromise ? "Promise<" : ""}${(0, __strings_js.nicify)(result)}${isPromise ? ">" : ""}`, {
					this: this,
					args,
				...
				const error = (e, isPromise) => console.debug(`logged(...): Error in ${name}.${String(prop)}(${args.map((a) => (0, __strings_js.nicify)(a)).join(", ")})`, {
					this: this,
					args,
					promise: isPromise ? result : false,
					error: e,
...
exports.throwingProxy = throwingProxy;
