...
let __functions_js = require("./functions.js");
...
function deepPlainEquals(obj1, obj2, options = {}) {
	...
	switch (typeof obj1) {
		...
		default: throw new Error("Unexpected typeof " + typeof obj1);
	}
}
(void 0)?.test("deepPlainEquals", ({ expect }) => {
	...
	expect(deepPlainEquals({
		a: 1,
		b: void 0
	}, { a: 1 }, { ignoreUndefinedValues: true })).toBe(true);
	expect(deepPlainEquals({
		a: 1,
		b: void 0
	}, { a: 1 })).toBe(false);
...
function shallowClone(obj) {
	if (!isCloneable(obj)) throw new __errors_js.StackAssertionError("shallowClone does not support symbols or functions", { obj });
...
function deepPlainClone(obj) {
	if (typeof obj === "function") throw new __errors_js.StackAssertionError("deepPlainClone does not support functions");
	if (typeof obj === "symbol") throw new __errors_js.StackAssertionError("deepPlainClone does not support symbols");
...
function deepMerge(baseObj, mergeObj) {
	...
	].some((o) => !isCloneable(o))) throw new __errors_js.StackAssertionError("deepMerge does not support functions or symbols", {
		baseObj,
		mergeObj
	});
...
(void 0)?.test("typedFromEntries", ({ expect }) => {
	...
	expect(typedFromEntries([["a", 1], ["b", 2]])).toEqual({
		a: 1,
		b: 2
	});
...
function omit(obj, keys) {
	if (!Array.isArray(keys)) throw new __errors_js.StackAssertionError("omit: keys must be an array", {
		obj,
		keys
	});
	return Object.fromEntries(Object.entries(obj).filter(([k]) => !keys.includes(k)));
...
(void 0)?.test("split", ({ expect }) => {
	...
	expect(split(obj, ["a", "e"])).toEqual([{ a: 1 }, {
		b: 2,
		c: 3,
		d: 4
	}]);
...
(void 0)?.test("deepSortKeys", ({ expect }) => {
	...
	expect(Object.entries(sorted.b)).toEqual([["c", 2], ["d", 3]]);
	expect(Object.entries(sorted.e[2])).toEqual([["f", 6], ["g", 7]]);
	expect(Object.entries(sorted.h)).toEqual([["i", {
		j: 8,
		k: 9
	}], ["l", 10]]);
	expect(Object.entries(sorted.h.i)).toEqual([["j", 8], ["k", 9]]);
});
function set(obj, key, value) {
	if (!isObjectLike(obj)) throw new __errors_js.StackAssertionError(`set: obj is not an object (found: ${obj === null ? "null" : typeof obj})`, {
		obj,
		key,
		value
	});
...
function get(obj, key) {
	if (obj == null) throw new __errors_js.StackAssertionError("get: obj is null or undefined", {
		obj,
		key
	});
	const descriptor = Object.getOwnPropertyDescriptor(obj, key);
	if (!descriptor) throw new __errors_js.StackAssertionError(`get: key ${String(key)} does not exist`, {
		obj,
		key
	});
...
function getOrUndefined(obj, key) {
	if (obj == null) throw new __errors_js.StackAssertionError("getOrUndefined: obj is null or undefined", {
		obj,
		key
	});
...
function has(obj, key) {
	if (obj == null) throw new __errors_js.StackAssertionError("has: obj is null or undefined", {
		obj,
		key
	});
...
function deleteKey(obj, key) {
	...
	else throw new __errors_js.StackAssertionError(`deleteKey: key ${String(key)} does not exist`, {
		obj,
		key
	});
}
