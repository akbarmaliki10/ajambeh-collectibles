...
//#region src/utils/paginated-lists.tsx
/**
...
var PaginatedList = class {
	...
	*/
	async nextOrPrev(type, options) {
		...
		for (let i = 1; i < result.length; i++) if (this._compare(options.orderBy, result[i].item, result[i - 1].item) < 0) throw new __errors_js.StackAssertionError("Paginated list result is not sorted; something is wrong with the implementation", {
			i,
			options,
			result
		});
		if (["exact", "at-most"].includes(options.limitPrecision) && result.length > options.limit) if (type === "next") {
	...
	*/
	flatMap(options) {
		const that = this;
		class FlatMapPaginatedList extends PaginatedList {
			_getFirstCursor() {
			...
			async _nextOrPrev(type, { limit, filter, orderBy, cursor }) {
				const estimatedItems = options.estimateItemsToFetch({
					limit,
					filter,
					orderBy
				});
				...
				return {
					items: original.items.flatMap((itemEntry) => options.itemMapper(itemEntry, filter, orderBy)),
	...
	map(options) {
		return this.flatMap({
			itemMapper: (itemEntry, filter, orderBy) => {
				return [{
					item: options.itemMapper(itemEntry.item),
					prevCursor: itemEntry.prevCursor,
					nextCursor: itemEntry.nextCursor
				}];
			},
	...
	*/
	static merge(...lists) {
		class MergePaginatedList extends PaginatedList {
			_getFirstCursor() {
			...
			_compare(orderBy, a, b) {
				...
				if (!listsResults.every((result) => result === listsResults[0])) throw new __errors_js.StackAssertionError("Lists have different compare results; make sure that they use the same compare function", {
					lists,
					listsResults,
					orderBy,
					a,
			...
			async _nextOrPrev(type, { limit, filter, orderBy, cursor }) {
				...
				const sortedItems = [...fetchedLists.flatMap((list, i) => list.items.map((itemEntry) => ({
					itemEntry,
					listIndex: i
				})))].sort((a, b) => this._compare(orderBy, a.itemEntry.item, b.itemEntry.item));
				...
				if (type === "prev") sortedItemsWithMergedCursors.reverse();
				return {
	...
	*/
	static empty() {
		class EmptyPaginatedList extends PaginatedList {
			_getFirstCursor() {
				return "first";
			}
			_getLastCursor() {
...
var ArrayPaginatedList = class extends PaginatedList {
	...
	async _nextOrPrev(type, options) {
		const sortedArray = [...this.array.filter(options.filter)].sort((a, b) => this._compare(options.orderBy, a, b));
		...
		return {
			items: itemEntriesArray.slice(Math.min(clampedOldCursor, newCursor), Math.max(clampedOldCursor, newCursor)),
...
//#endregion
exports.ArrayPaginatedList = ArrayPaginatedList;
