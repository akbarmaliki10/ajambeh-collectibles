Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });
const require_chunk = require('../chunk-BE-pF4vm.js');
let __bytes_js = require("./bytes.js");

//#region src/utils/http.tsx
const HTTP_METHODS = {
	"GET": {
		safe: true,
		idempotent: true
	},
	"POST": {
		safe: false,
		idempotent: false
	},
	"PUT": {
		safe: false,
		idempotent: true
	},
	"DELETE": {
		safe: false,
		idempotent: true
	},
	"PATCH": {
		safe: false,
		idempotent: false
	},
	"OPTIONS": {
		safe: true,
		idempotent: true
	},
	"HEAD": {
		safe: true,
		idempotent: true
	},
	"TRACE": {
		safe: true,
		idempotent: true
	},
	"CONNECT": {
		safe: false,
		idempotent: false
	}
};
function decodeBasicAuthorizationHeader(value) {
	const [type, encoded, ...rest] = value.split(" ");
	if (rest.length > 0) return null;
	if (!encoded) return null;
	if (type !== "Basic") return null;
	if (!(0, __bytes_js.isBase64)(encoded)) return null;
	const split = new TextDecoder().decode((0, __bytes_js.decodeBase64)(encoded)).split(":");
	return [split[0], split.slice(1).join(":")];
}
(void 0)?.test("decodeBasicAuthorizationHeader", ({ expect }) => {
	const username = "user";
	const password = "pass";
	expect(decodeBasicAuthorizationHeader(encodeBasicAuthorizationHeader(username, password))).toEqual([username, password]);
	const complexPassword = "pass:with:colons";
	expect(decodeBasicAuthorizationHeader(encodeBasicAuthorizationHeader(username, complexPassword))).toEqual([username, complexPassword]);
	expect(decodeBasicAuthorizationHeader("NotBasic dXNlcjpwYXNz")).toBe(null);
	expect(decodeBasicAuthorizationHeader("Basic")).toBe(null);
	expect(decodeBasicAuthorizationHeader("Basic not-base64")).toBe(null);
	expect(decodeBasicAuthorizationHeader("Basic dXNlcjpwYXNz extra")).toBe(null);
});
function encodeBasicAuthorizationHeader(id, password) {
	if (id.includes(":")) throw new Error("Basic authorization header id cannot contain ':'");
	return `Basic ${(0, __bytes_js.encodeBase64)(new TextEncoder().encode(`${id}:${password}`))}`;
}
(void 0)?.test("encodeBasicAuthorizationHeader", ({ expect }) => {
	expect(encodeBasicAuthorizationHeader("user", "pass")).toMatch(/^Basic [A-Za-z0-9+/=]+$/);
	expect(encodeBasicAuthorizationHeader("user", "")).toMatch(/^Basic [A-Za-z0-9+/=]+$/);
	expect(encodeBasicAuthorizationHeader("user", "p@ss!w0rd")).toMatch(/^Basic [A-Za-z0-9+/=]+$/);
	expect(() => encodeBasicAuthorizationHeader("user:name", "pass")).toThrow();
});

//#endregion
exports.HTTP_METHODS = HTTP_METHODS;
exports.decodeBasicAuthorizationHeader = decodeBasicAuthorizationHeader;
exports.encodeBasicAuthorizationHeader = encodeBasicAuthorizationHeader;
//# sourceMappingURL=http.js.map