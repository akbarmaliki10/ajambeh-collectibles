...
function rejected(reason) {
	...
	if (rejectedCache.has([reason])) return rejectedCache.get([reason]);
...
async function wait(ms) {
	if (!Number.isFinite(ms) || ms < 0) throw new __errors_js.StackAssertionError(`wait() requires a non-negative integer number of milliseconds to wait. (found: ${ms}ms)`);
	if (ms >= 2 ** 31) throw new __errors_js.StackAssertionError("The maximum timeout for wait() is 2147483647ms (2**31 - 1). (found: ${ms}ms)");
...
(void 0)?.test("waitUntil", async ({ expect }) => {
	...
	try {
		...
		expect(error).toBeInstanceOf(__errors_js.StackAssertionError);
...
function runAsynchronouslyWithAlert(...args) {
	return runAsynchronously(args[0], {
		...args[1],
		onError: (error) => {
			if (___index_js.KnownError.isKnownError(error) && typeof process !== "undefined" && process.env.NODE_ENV?.includes("production")) alert(error.message);
			else alert(`An unhandled error occurred. Please ${process.env.NODE_ENV === "development" ? `check the browser console for the full error.` : "report this to the developer."}\n\n${error}`);
...
function runAsynchronously(promiseOrFunc, options = {}) {
	...
	if (promiseOrFunc) {
		...
		promiseOrFunc.catch((error) => {
			...
			const newError = new __errors_js.StackAssertionError("Uncaught error in asynchronous function: " + (0, __errors_js.errorToNiceString)(error), { cause: error });
...
async function timeout(promiseOrFunc, ms) {
	...
	return await Promise.race([promise.then((value) => __results_js.Result.ok(value)), wait(ms).then(() => __results_js.Result.error(new TimeoutError(ms)))]);
}
