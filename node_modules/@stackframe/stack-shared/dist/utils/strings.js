...
let __arrays_js = require("./arrays.js");
...
function typedToLowercase(s) {
	if (typeof s !== "string") throw new __errors_js.StackAssertionError("Expected a string for typedToLowercase", { s });
...
function typedToUppercase(s) {
	if (typeof s !== "string") throw new __errors_js.StackAssertionError("Expected a string for typedToUppercase", { s });
...
function stringCompare(a, b) {
	if (typeof a !== "string" || typeof b !== "string") throw new __errors_js.StackAssertionError(`Expected two strings for stringCompare, found ${typeof a} and ${typeof b}`, {
		a,
		b
	});
...
function templateIdentity(strings, ...values) {
	if (values.length !== strings.length - 1) throw new __errors_js.StackAssertionError("Invalid number of values; must be one less than strings", {
		strings,
		values
	});
...
*/
const nicifiableClassNameOverrides = new Map(Object.entries({ Headers }).map(([k, v]) => [v, k]));
      ...
      `;
		...
		case "object": {
			...
			if (Array.isArray(value)) {
				...
				if (resValues.length !== resValueLength) throw new __errors_js.StackAssertionError("nicify of object: resValues.length !== resValueLength", {
					value,
					resValues,
					resValueLength
				});
		...
		default: return `${typeof value}<${value}>`;
	}
}
function replaceAll(input, searchValue, replaceValue) {
	if (searchValue === "") throw new __errors_js.StackAssertionError("replaceAll: searchValue is empty");
...
function getNicifiableEntries(value) {
	...
	if (isRecordLike(value)) return [...value.entries()].sort(([a], [b]) => stringCompare(`${a}`, `${b}`));
