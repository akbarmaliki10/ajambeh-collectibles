import { r as UserIdentity, t as GenericQueryCtx } from "../../storage-kTmOAWHW.js";
import { ProviderType } from "@stackframe/stack-shared/dist/utils/oauth";

//#region src/lib/stack-app/common.d.ts
type RedirectToOptions = {
  replace?: boolean;
  noRedirectBack?: boolean;
};
type AsyncStoreProperty<Name extends string, Args extends any[], Value, IsMultiple extends boolean> = { [key in `${IsMultiple extends true ? "list" : "get"}${Capitalize<Name>}`]: (...args: Args) => Promise<Value> } & { [key in `use${Capitalize<Name>}`]: (...args: Args) => Value };
type EmailConfig = {
  host: string;
  port: number;
  username: string;
...
type HandlerPageUrls = Record<"handler" | "signIn" | "signUp" | "signOut" | "emailVerification" | "passwordReset" | "forgotPassword" | "oauthCallback" | "magicLinkCallback" | "accountSettings" | "teamInvitation" | "mfa" | "error" | "onboarding", string | {
  type: "custom";
  url: string;
  version: number;
} | {
...
type DefaultHandlerUrlTarget = string | {
  type: "hosted" | "handler-component";
};
...
type OAuthScopesOnSignIn = { [key in ProviderType]: string[] };
/**
...
//#endregion
export { AsyncStoreProperty, AuthLike, ConvexCtx, DefaultHandlerUrlTarget, EmailConfig, GetCurrentPartialUserOptions, GetCurrentUserOptions, HandlerPageUrls, HandlerRedirectUrls, HandlerUrlOptions, HandlerUrlTarget, HandlerUrls, OAuthScopesOnSignIn, RedirectMethod, RedirectToOptions, RequestLike, ResolvedHandlerUrls, TokenStoreInit, stackAppInternalsSymbol };
