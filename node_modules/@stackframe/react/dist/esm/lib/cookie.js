...
function ensureClient() {
	if (!isBrowserLike()) throw new Error("cookieClient functions can only be called in a browser environment, yet window is undefined");
}
...
async function createCookieHelper() {
	if (isBrowserLike()) return createBrowserCookieHelper();
...
async function isSecure() {
	if (isBrowserLike()) return determineSecureFromClientContext();
	return false;
}
...
function deleteCookieClientInternal(name, options) {
	for (const partitioned of [true, false]) {
		if (options.domain !== void 0) Cookies.remove(name, {
			domain: options.domain,
...
//#endregion
export { consumeVerifierAndStateCookie, createBrowserCookieHelper, createCookieHelper, createPlaceholderCookieHelper, deleteCookie, deleteCookieClient, getAllCookiesClient, getCookie, getCookieClient, isSecure, saveVerifierAndState, setCookie, setCookieClient, setOrDeleteCookie, setOrDeleteCookieClient };
