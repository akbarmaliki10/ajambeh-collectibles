...
function hasCrossDomainHandoffParams(url) {
	return url.searchParams.has(crossDomainAuthQueryParams.state) && url.searchParams.has(crossDomainAuthQueryParams.codeChallenge) && url.searchParams.has(crossDomainAuthQueryParams.afterCallbackRedirectUrl);
}
function buildCrossDomainAuthCallbackUrl(options) {
	...
	if (localOAuthCallbackUrl.origin !== options.currentUrl.origin) throw new StackAssertionError("Cross-domain auth callback URL must stay on the current origin", {
	...
	if (options.state != null) localOAuthCallbackUrl.searchParams.set(crossDomainAuthQueryParams.state, options.state);
	if (options.codeChallenge != null) localOAuthCallbackUrl.searchParams.set(crossDomainAuthQueryParams.codeChallenge, options.codeChallenge);
	if (options.afterCallbackRedirectUrl != null) localOAuthCallbackUrl.searchParams.set(crossDomainAuthQueryParams.afterCallbackRedirectUrl, options.afterCallbackRedirectUrl);
...
function buildRedirectBackAwareHandlerUrl(options) {
	...
	for (const preservedParam of [
		...
		if (currentValue != null && !nextUrl.searchParams.has(preservedParam)) nextUrl.searchParams.set(preservedParam, currentValue);
	}
	if (options.handlerName === "signOut") {
		if (!nextUrl.searchParams.has("after_auth_return_to")) if (options.currentUrl.protocol === nextUrl.protocol && options.currentUrl.host === nextUrl.host) nextUrl.searchParams.set("after_auth_return_to", getRelativePart(options.currentUrl));
	...
	} else if (options.currentUrl.protocol === nextUrl.protocol && options.currentUrl.host === nextUrl.host && !nextUrl.searchParams.has("after_auth_return_to")) nextUrl.searchParams.set("after_auth_return_to", getRelativePart(options.currentUrl));
	return nextUrl.origin === options.currentUrl.origin ? getRelativePart(nextUrl) : nextUrl.toString();
}
...
async function planRedirectToHandler(options) {
	...
	if (options.handlerName !== "signIn" && options.handlerName !== "signUp" && options.handlerName !== "onboarding" && options.handlerName !== "signOut") throw new StackAssertionError("Unexpected redirect-back-aware handler policy mismatch", {
		handlerName: options.handlerName,
		policy
	});
	return {
...
function readCrossDomainHandoffParams(currentUrl, redirectBackTarget) {
	return {
		state: currentUrl.searchParams.get(crossDomainAuthQueryParams.state) ?? redirectBackTarget.searchParams.get(crossDomainAuthQueryParams.state),
		codeChallenge: currentUrl.searchParams.get(crossDomainAuthQueryParams.codeChallenge) ?? redirectBackTarget.searchParams.get(crossDomainAuthQueryParams.codeChallenge),
		afterCallbackRedirectUrl: currentUrl.searchParams.get(crossDomainAuthQueryParams.afterCallbackRedirectUrl) ?? redirectBackTarget.searchParams.get(crossDomainAuthQueryParams.afterCallbackRedirectUrl)
	};
}
...
//#endregion
export { crossDomainAuthQueryParams, getCrossDomainHandoffParamsFromCurrentUrl, planRedirectToHandler };
