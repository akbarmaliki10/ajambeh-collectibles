...
import { clientVersion, createCache, getBaseUrl, getDefaultExtraRequestHeaders, getDefaultProjectId, getDefaultPublishableClientKey, getDefaultSecretServerKey, getDefaultSuperSecretAdminKey, resolveConstructorOptions, useAsyncCache } from "./common.js";
...
var _StackAdminAppImplIncomplete = class extends _StackServerAppImplIncomplete {
	constructor(options, extraOptions) {
		...
		super(resolvedOptions, {
			...
			interface: extraOptions?.interface ?? new StackAdminInterface({
				...
				clientVersion,
				...resolvedOptions.projectOwnerSession ? { projectOwnerSession: resolvedOptions.projectOwnerSession } : {
		...
		this._adminEmailDraftsCache = createCache(async () => {
		...
		this._metricsCache = createCache(async ([includeAnonymous]) => {
		...
		this._emailPreviewWithEditableMarkersCache = createCache(async ([themeId, themeTsxSource, templateId, templateTsxSource, editableSource]) => {
			return await this._interface.renderEmailPreview({
				themeId,
				themeTsxSource,
				templateId,
		...
		this._stripeAccountInfoCache = createCache(async () => {
			try {
		...
		this._transactionsCache = createCache(async ([cursor, limit, type, customerType]) => {
	...
	_adminOwnedProjectFromCrud(data, onRefresh) {
		if (this._tokenStoreInit !== null) throw new StackAssertionError("Owned apps must always have tokenStore === null — did you not create this project with app._createOwnedApp()?");
		return {
	...
	_adminProjectFromCrud(data, onRefresh) {
		if (data.id !== this.projectId) throw new StackAssertionError(`The project ID of the provided project JSON (${data.id}) does not match the project ID of the app (${this.projectId})!`);
		const app = this;
		return {
			id: data.id,
			...
			async update(update) {
				...
				const hasConfigUpdate = !!updateOptions.config && typedValues(updateOptions.config).some((value) => value !== void 0);
				if (typedEntries(updateOptions).some(([key, value]) => {
				...
				if (requirePublishableClientKey !== void 0) {
					await app._interface.updateConfigOverride("project", { "project.requirePublishableClientKey": requirePublishableClientKey });
					await app._refreshProjectConfig();
				}
			},
			async delete() {
	...
	async getProject() {
		return this._adminProjectFromCrud(Result.orThrow(await this._adminProjectCache.getOrWait([], "write-only")), () => this._refreshProject());
	}
	useProject() {
	...
	async listInternalApiKeys() {
		return Result.orThrow(await this._internalApiKeysCache.getOrWait([], "write-only")).map((j) => this._createInternalApiKeyFromCrud(j));
	}
	useInternalApiKeys() {
	...
	async listEmailThemes() {
		return Result.orThrow(await this._adminEmailThemesCache.getOrWait([], "write-only")).map((theme) => ({
			id: theme.id,
			displayName: theme.display_name
		}));
	}
	async listEmailTemplates() {
		return Result.orThrow(await this._adminEmailTemplatesCache.getOrWait([], "write-only")).map((template) => ({
			id: template.id,
			displayName: template.display_name,
			themeId: template.theme_id,
			tsxSource: template.tsx_source
	...
	async listEmailDrafts() {
		return Result.orThrow(await this._adminEmailDraftsCache.getOrWait([], "write-only")).map((draft) => ({
			id: draft.id,
			displayName: draft.display_name,
			themeId: draft.theme_id,
			tsxSource: draft.tsx_source,
	...
	async listTeamPermissionDefinitions() {
		return Result.orThrow(await this._adminTeamPermissionDefinitionsCache.getOrWait([], "write-only")).map((p) => this._serverTeamPermissionDefinitionFromCrud(p));
	}
	...
	async listProjectPermissionDefinitions() {
		return Result.orThrow(await this._adminProjectPermissionDefinitionsCache.getOrWait([], "write-only")).map((p) => this._serverProjectPermissionDefinitionFromCrud(p));
	}
	...
	async _refreshProjectConfig() {
		await Promise.all([this._configOverridesCache.refresh([]), this._adminProjectCache.refresh([])]);
	}
	...
	get [stackAppInternalsSymbol]() {
		return {
			...
			useMetrics: (includeAnonymous = false) => {
				return useAsyncCache(this._metricsCache, [includeAnonymous], "adminApp.useMetrics()");
			}
		};
	}
	async sendTestEmail(options) {
		...
		if (response.success) return Result.ok(void 0);
		else return Result.error({ errorMessage: response.error_message ?? throwErr("Email test error not specified") });
	}
	async sendTestWebhook(options) {
		const response = await this._interface.sendTestWebhook({ endpoint_id: options.endpointId });
		if (response.success) return Result.ok(void 0);
		else return Result.error({ errorMessage: response.error_message ?? throwErr("Webhook test error not specified") });
	}
	...
	async checkManagedEmailStatus(options) {
		return { status: (await this._interface.checkManagedEmailStatus({
	...
	async applyManagedEmailProvider(options) {
		const result = await this._interface.applyManagedEmailProvider({ domain_id: options.domainId });
	...
	async deleteEmailDraft(id) {
		...
		if (current.status === "ok" && current.data.status === "ok") this._adminEmailDraftsCache.forceSetCachedValue([], Result.ok(current.data.data.filter((d) => d.id !== id)));
		await this._adminEmailDraftsCache.refresh([]);
	}
	...
	async rewriteTemplateSourceWithAI(templateTsxSource) {
		return { tsxSource: (await this._interface.rewriteTemplateSourceWithAI(templateTsxSource)).tsx_source };
	}
	...
	useEmailPreviewWithEditableMarkers(options) {
		const crud = useAsyncCache(this._emailPreviewWithEditableMarkersCache, [
			options.themeId,
			options.themeTsxSource,
			options.templateId,
			options.templateTsxSource,
	...
	useEmailTheme(id) {
		const crud = useAsyncCache(this._adminEmailThemeCache, [id], "adminApp.useEmailTheme()");
		return {
	...
	async createItemQuantityChange(options) {
		await this._interface.updateItemQuantity({
			...
			..."userId" in options ? { userId: options.userId } : "teamId" in options ? { teamId: options.teamId } : { customCustomerId: options.customCustomerId }
		}, {
	...
	async listTransactions(params) {
		return Result.orThrow(await this._transactionsCache.getOrWait([
			params.cursor,
			params.limit,
			params.type,
			params.customerType
	...
	_emailOutboxCrudToAdmin(crud) {
		...
		return (() => {
			switch (crud.status) {
				...
				default: throw new StackAssertionError(`Unknown email outbox status: ${crud.status}`, { status: crud.status });
			}
		})();
	}
	...
	useTransactions(params) {
		return useAsyncCache(this._transactionsCache, [
			params.cursor,
			params.limit,
			params.type,
			params.customerType
	...
	async previewAffectedUsersByOnboardingChange(onboarding, limit) {
		const result = await this._interface.previewAffectedUsersByOnboardingChange({ require_email_verification: onboarding.requireEmailVerification }, limit);
		return {
...
//#endregion
export { _StackAdminAppImplIncomplete };
